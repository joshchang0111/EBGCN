...
def train_model(treeDic, x_test, x_train, args, iter):
	...
	for epoch in range(args.n_epochs):
		traindata_list, testdata_list = loadData(args, args.datasetname, treeDic, x_train, x_test, args.TDdroprate, args.BUdroprate)
		...
		for Batch_data in tqdm(test_loader):
			...
			if args.num_class == 3:
				...
				raise NotImplementedError("--num_class == {} is not implemented!".format(args.num_class))
		...
		early_stopping(np.mean(temp_val_losses), np.mean(temp_val_accs), np.mean(temp_val_F1), np.mean(temp_val_F2),
					   np.mean(temp_val_F3), np.mean(temp_val_F4), model, args.modelname, args.datasetname)
...
if __name__ == '__main__':
	...
	parser.add_argument('--modelname', type=str, default="BiGCN", metavar='modeltype', help='model type, option: BiGCN/EBGCN')
	parser.add_argument('--input_features', type=int, default=5000, metavar='inputF', help='dimension of input features (TF-IDF)')
	parser.add_argument('--hidden_features', type=int, default=64, metavar='graph_hidden', help='dimension of graph hidden state')
	...
	parser.add_argument('--num_workers', type=int, default=30, metavar='num_workers', help='number of workers for training')

	# Parameters for training the model
	parser.add_argument('--seed', type=int, default=2020, help='random state seed')
	parser.add_argument('--no_cuda', action='store_true', help='does not use GPU')
	parser.add_argument('--num_cuda', type=int, default=0, help='index of GPU 0/1')
	...
	parser.add_argument('--iterations', type=int, default=10, metavar='F', help='number of iterations for 5-fold cross-validation')
	...
	## NEW
	parser.add_argument("--data_root", type=str, default="/mnt/hdd1/projects/BiGCN/dataset/processedV2", help="Use same dataset as BiGCN (processedV2 from RumorV2)")
	parser.add_argument("--output_root", type=str, default="/mnt/hdd1/projects/EBGCN")
	parser.add_argument("--n_fold", type=str, default=5)
	...
	for iter in range(args.iterations):
		...
		## Load fixed 5-fold
		fold_tests, fold_trains = [], []
		for fold_idx in range(args.n_fold):
			fold_test, fold_train = loadfoldlist(args, args.datasetname, fold_idx, seed=args.seed)
			fold_tests.append(fold_test)
			fold_trains.append(fold_train)

		accs, NR_F1, FR_F1, TR_F1, UR_F1 = [], [], [], [], []
		for fold_idx in range(args.n_fold):
			print("\n{}: Fold [{}]".format(args.datasetname, fold_idx))
			fold_timestamp = time()
		...
		print("Acc:{:.4f}\tNR_F1:{:.4f}\tFR_F1:{:.4f}\tTR_F1:{:.4f}\tUR_F1:{:.4f}\t\tavg_F1:{:.4f}".format(
			np.mean(accs),
			np.mean(NR_F1),
			np.mean(FR_F1),
			np.mean(TR_F1),
	...
	## Result of all iterations (average)
	print("****  Total Result  ****")
	print("Acc:{:.4f}\tNR_F1:{:.4f}\tFR_F1:{:.4f}\tTR_F1:{:.4f}\tUR_F1:{:.4f}".format(
		np.mean(total_accs),
		np.mean(total_NR_F1),
		np.mean(total_FR_F1),
		np.mean(total_TR_F1),
